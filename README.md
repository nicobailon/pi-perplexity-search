...
Mistral uses the same credential-source rules through `mistralApiKey`; without an explicit source, `MISTRAL_API_KEY` is used. `mistralSearchModel` defaults to `mistral-small-latest` and must be a non-empty string. `mistralSearchTool` defaults to `web_search` and accepts only `web_search` or the opt-in `web_search_premium` tool.
