...
Successful provider answers are kept in labeled sections for the Curator summary model, source URLs and inline content are deduplicated, and one provider failure does not discard the other results. If every participating provider fails, the tool returns per-provider diagnostics. In the Curator, **All** can also be selected like the other provider buttons. Each query remains one Curator item containing the combined provider output, so the approved summary is what Pi receives.
